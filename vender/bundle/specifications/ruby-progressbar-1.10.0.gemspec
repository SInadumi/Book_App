
# stub: ruby-progressbar 1.10.0 ruby lib

Gem::Specification.new do |s|
  s.name = 'ruby-progressbar'
  s.version = '1.10.0'

  s.required_rubygems_version = Gem::Requirement.new('>= 0') if s.respond_to? :required_rubygems_version=
  s.metadata = { 'allowed_push_host' => 'https://rubygems.org', 'bug_tracker_uri' => 'https://github.com/jfelchner/ruby-progressbar/issues', 'changelog_uri' => 'https://github.com/jfelchner/ruby-progressbar/CHANGELOG.md', 'documentation_uri' => 'https://github.com/jfelchner/ruby-progressbar/tree/releases/v1.10.0', 'homepage_uri' => 'https://github.com/jfelchner/ruby-progressbar', 'source_code_uri' => 'https://github.com/jfelchner/ruby-progressbar', 'wiki_uri' => 'https://github.com/jfelchner/ruby-progressbar/wiki' } if s.respond_to? :metadata=
  s.require_paths = ['lib']
  s.authors = %w[thekompanee jfelchner]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\nEw38fD/eb7KjFt6HWjaE88pp12uYoAUV0xTysHhml+rk1/rn/h3T1Exa\n-----END CERTIFICATE-----\n"]
  s.date = '2018-08-03'
  s.description = 'Ruby/ProgressBar is an extremely flexible text progress bar library for Ruby. The output can be customized with a flexible formatting system including: percentage, bars of various formats, elapsed time and estimated time remaining.'
  s.email = ['support@thekompanee.com']
  s.homepage = 'https://github.com/jfelchner/ruby-progressbar'
  s.licenses = ['MIT']
  s.rubygems_version = '2.5.1'
  s.summary = 'Ruby/ProgressBar is a flexible text progress bar library for Ruby.'

  s.installed_by_version = '2.5.1' if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0')
      s.add_development_dependency('rspec', ['~> 3.7'])
      s.add_development_dependency('rspectacular', ['~> 0.70.6'])
      s.add_development_dependency('fuubar', ['~> 2.3'])
      s.add_development_dependency('warning_filter', ['~> 0.0.6'])
      s.add_development_dependency('timecop', ['= 0.6.0'])
    else
      s.add_dependency('rspec', ['~> 3.7'])
      s.add_dependency('rspectacular', ['~> 0.70.6'])
      s.add_dependency('fuubar', ['~> 2.3'])
      s.add_dependency('warning_filter', ['~> 0.0.6'])
      s.add_dependency('timecop', ['= 0.6.0'])
    end
  else
    s.add_dependency('rspec', ['~> 3.7'])
    s.add_dependency('rspectacular', ['~> 0.70.6'])
    s.add_dependency('fuubar', ['~> 2.3'])
    s.add_dependency('warning_filter', ['~> 0.0.6'])
    s.add_dependency('timecop', ['= 0.6.0'])
  end
end
